...
def perform_a_normalised_cut(image_indc, _weights, thresh, search_point_count=20, k=2):
    ...
    eig_val, eig_vec = eigsh((D-_weights), k=k, Minv=D_inv, M=D, which='SM', tol=0.0001)
...
def normalised_cuts(img, thresh=0.04, r_sq=5**2, sigma_x_sq=3**2, sigma_i_sq=0.07**2, search_point_count=20,
                    region_min=10, k=2):
    scale = 7500.0/(img.shape[0]*img.shape[1])
    if scale < 1.0:
    ...
    img_flat = scaled_img.reshape((-1, scaled_img.shape[-1]))
    ind = np.indices(scaled_img.shape[:2]).reshape((2, -1)).T
    weights = calculate_weights(img_flat, ind, r_sq, sigma_x_sq, sigma_i_sq)
